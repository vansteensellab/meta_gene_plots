...
file_list = list.files('results_supercoiling/window_runmean',
                       pattern='border.*-240000-24000-5000',
                       full.names=T)
meta.dt = data.table(str_match(file_list, '.*/(.*)_border-(.*)_(.*?)-.*'))
...
binned.dt = meta.dt[condition%in%c('WT', 'RPE_CT', 'RPE_DRB', 'RPE_SC',
                                   'RPE_Top1'),
                    fread(file, stringsAsFactors=F, header=T), by=file][,-'transcript_ID']


file_mean = binned.dt[,list(pos = as.numeric(colnames(.SD)),
...
plot_list = apply(unique(mean_rep.dt[,c('region', 'target')]),
                  1, function(x){
                      ...
                      ggplot(dt, aes(x=pos, y=log2_mean, color=condition)) +
                          ...
                          theme_bw() +
                          scale_x_continuous(lim=c(-200000, 200000), name="Position",
                                             labels = scales::comma)
                  })
pdf('cl20200609_border_profiles.pdf', useDingbats=F)
